...
// Activity 1: Basic Event Handling
    ...
    document.querySelector("#change").addEventListener("click",()=>{
        document.querySelector("#para").textContent = "Hello this is maksud alam."
    })

    // Task 2: Add a double-click event listener to an image that toggles its visibility.
    let toggleImg = document.querySelector("#dblImage")
    let toggleBtn = document.querySelector("#hide")
    toggleBtn.addEventListener("dblclick",()=>{
        if(toggleImg.style.display === "none"){
            toggleImg.style.display = "block";
            toggleBtn.textContent = "Double click to hide";
        } else {
            toggleImg.style.display = "none";
            toggleBtn.textContent = "Double click to show hidden img";
        }
    })
...
// Activity 2: Mouse Events
    ...
    let mouseMoveDiv = document.querySelector("#moveEvent");
    mouseMoveDiv.addEventListener("mousemove",()=>{
        mouseMoveDiv.style.backgroundColor = "red";
    })
    // Task 4: Add a mouseout event listener to an element that resets its background color.
    mouseMoveDiv.addEventListener("mouseleave",()=>{
        mouseMoveDiv.style.backgroundColor = "#90ee90";
    })

// Activity 3: Keyboard Events
    // Task 5: Add a keydown event listener to an input field that logs the key pressed to the console.
    let logByKeyDown = document.querySelector("#keyDownEvent");
    logByKeyDown.addEventListener("keydown",(key)=>{
        console.log(key);
    })
    // Task 6: Add a keyup event listener to an input field that displays the current value in a paragraph.
    let anpara = document.querySelector("#anpara")
    logByKeyDown.addEventListener("keyup",(key)=>{
        anpara.textContent = logByKeyDown.value;
    })
